...
- Modularizacion iniciada: `src/lib/money.ts`, `src/lib/dates.ts`, `src/lib/audit.ts`, `src/lib/storage.ts`, `src/lib/display.ts`, `src/lib/ids.ts`, `src/lib/machineHistory.ts`, `src/lib/currentAccounts.ts`, `src/lib/accountMovements.ts`, `src/lib/cashTotals.ts`, `src/lib/differences.ts`, `src/lib/salaryRules.ts` y `src/lib/sorting.ts` ya contienen reglas compartidas. `src/components/ui.tsx` contiene `InfoCard`, `FormButtons` y `Modal`. `src/features/manager/Differences.tsx`, `src/features/cashier/OpenCash.tsx`, `src/features/cashier/ClosedBalanceSummary.tsx`, `src/features/cashier/Counters.tsx` y `src/features/cashier/CloseCash.tsx` ya salieron de `src/App.tsx`. La mayor parte de pantallas y acciones de UI siguen en `src/App.tsx`.
